{ajaxheader modname='EZComments' filename='fade.js' effects=true}

<a href="javascript:fade();">{gt text='Comments'} ({$ezc_pager.numitems})</a>
        ...
        <li id="comment{$comment.id|safetext}" class="ezc_comment z-clearfix {cycle values='ezc_odd,ezc_even'}" >
            <ul class="ezc_metadata">
                ...
                <li class="ezc_options">
                    {if $msgmodule}
                    <a href="{modurl modname=$msgmodule type='user' func='inbox' func='newpm' uid=$comment.uid}">{img modname='core' src='mail_generic.png' set='icons/extrasmall' __title='Send mail to user' __alt='Send mail to user'}</a>
                    {/if}
                    {if $comment.del}
